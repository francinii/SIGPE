...
$sql = "SELECT  `nombreOficina`, `capacidadPermanente`, `capacidadTemporal`, `representanteComite`,"
        ...
        . ",`correoElectronico`,`sector` FROM `FormularioPoblacion` WHERE `FKidPlanEmergencias`=" . $idPlanEmergencia;
$res = seleccion($sql);
...
<div class="well well-sm">
    <p><b><?= $vocab["poblacion_titulo"] ?></b></p>
    <p><?= $vocab["recurso_humano_titulo_desc"] ?></p>
    <div align="right" style = "padding-right:0.5%;">
        <?php if ($editar) { ?>
            <span class="text-center" >
                <a class="btn btn-info" onclick="javascript: agregarFilaSector('<?= $vocab["symbol_delete"] ?>', '<?= $vocab["Menu_Datos_sin_guardar"] ?>');"><i class=" fa fa-plus text-info"></i> <?= $vocab["menu_add"] ?> <?= $vocab["inventario_sector"] ?>  </a>
            </span>
            <span class="text-center" >
                <a class="btn btn-success" onclick="javascript: agregarFilaPoblacion('<?= $vocab["symbol_delete"] ?>', '<?= $vocab["Menu_Datos_sin_guardar"] ?>');"><i class=" fa fa-plus text-success"></i> <?= $vocab["menu_add"] ?> <?= $vocab["inventario_fila"] ?>  </a>
            </span>
        <?php } ?>
    </div>
</div>
        ...
        <tbody>
            ...
            if (count($res) > 0) {
                ...
                for ($i = 0, $id = 0; $i < count($res); $i++, $id++) {
                    $sector = ((is_array($res[0])) ? $res[0]['sector'] : "sector nuevo");
                    if ($sectorActual != $sector) {
                        $sectorActual = $sector;
                        ?>
                        <tr  id="Sec-<?= $id ?>">                   
                            <td  style="align-items:center; background-color:lightblue" colspan="10">
                                <input style="width:40%; margin: 0 auto;" type="text"  <?= (!$editar) ? "readonly" : ""; ?>  class="form-control requerido cambios" id="Sector<?= $id ?>" value="<?= (is_array($res[0])) ? $res[0]['sector'] : "sector nuevo"; ?>" ></td>
...
<?php if ($editar) { ?>
        ...
        <span class="text-center">
            <a class="btn btn-success"  onclick="javascript:guardarPoblacion('<?= $idPlanEmergencia ?>', 1)" name="submit"><i class="fa fa-rotate-left"></i> <?= $vocab["symbol_save"] . " " . $vocab["datos_generares_siguente"] ?></a>
        </span>
<?php } ?>
</div>
