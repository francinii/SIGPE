    ...
    <div class="col-lg-8 col-md-8 col-sm-11 col-xs-12">
        <form method="post" action="">            
            <div class="form-group">
                ...
                <input <?= ($view_mode == 0) ? "readonly" : ""; ?>  id="formulario_title" name="formulario_title" class="form-control" type="text" value="<?= $res[0]['titulo'] ?>"/> 
            ...
            <div class="form-group">
                ...
                <textarea <?= ($view_mode == 0) ? "readonly" : ""; ?>  class="ckeditor form-control" id="DescripcionArriba" name="DescripcionArriba" ><?= $res[0]['descripcionArriba'] ?></textarea>
                <p class="help-block"><small><?= $vocab["symbol_desc"] ?></small></p> 
            </div>
            <div class="form-group">
                <label  for="DescripcionAbajo"><?= $vocab["formulario_descripcion_abajo"] ?> </label>                
                <textarea <?= ($view_mode == 0) ? "readonly" : ""; ?>  class="ckeditor form-control" id="DescripcionAbajo" name="DescripcionAbajo" ><?= $res[0]['descripcionAbajo'] ?></textarea>
                <p class="help-block"><small><?= $vocab["symbol_desc"] ?></small></p> 
            </div>                       
            <?php
            if ($view_mode == 1) {
                if (check_permiso($mod4, $act4, $user_rol)) {
                    ...
                    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                        <a <?= ($view_mode == 0) ? "readonly" : ""; ?> class="btn btn-success btn-group-justified"  name="submit" onclick="update_formulario(<?= $res[0]['id'] ?>);"><i class="fa fa-save fa-inverse"></i> <?= $vocab["symbol_update"] . " " . $vocab["permits_title"] ?></a>
                    </div>
                    <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
