function agregarFilaIngresos(titulo, alert) {
    ...
    var fila = '<tr id="' + id + '">' +
            ...
            '<td> <input  type="text"     class="form-control  cambios" id="ubicacion' + id + '"></td>' +
...
function guardarIncresoCuerpoSocorro(idPlanEmergencia, pasar) {
    if (validate_Ingreso("#lista_ingreso")) {
        ...
        loading.innerHTML = cargando_bar;
        var ajax = NuevoAjax();
        var dimensionAreaAcceso = jQuery("#dimensionAreaAcceso").val();
        var radioGiro = jQuery("#radioGiro").val();
        var caseta = jQuery("#caseta").val();
        var plumas = jQuery("#plumas").val();
        var anchoLibre = jQuery("#anchoLibre").val();

        //Preparacion  llamada AJAX
        var _values_send =
                'idPlanEmergencia=' + idPlanEmergencia+
                 '&dimensionAreaAcceso=' + dimensionAreaAcceso+
                 '&radioGiro=' + radioGiro+
                 '&caseta=' + caseta+
                 '&plumas=' + plumas+
                 '&anchoLibre=' + anchoLibre;

        var _URL_ = "mod/planEmergencia/ajax_ingreso_cuerpo_socorro.php?";
        //alert(_URL_ + _values_send); //DEBUG
        ajax.open("POST", _URL_ + "&" + _values_send, true);
        ajax.onreadystatechange = function () {
            if (ajax.readyState == 1) {
                ...
                if (response == 0) {
                    ...
                    jAlert("Guardado  con exito", "Exito");
                    if (pasar) {
                        OpcionMenu('mod/planEmergencia/plan_emergencia_puestos_brigada.php?', 'idCentro=' + idCentro + '&nombreCentro=' + nombreCentro);
                    }
                } else if (response == 1 || response == 2) {
                    jAlert("Error en la Base de Datos, intente nuevamente.\n Si persiste informe a la USTDS", "Error");
                } else if (response == 3) {
